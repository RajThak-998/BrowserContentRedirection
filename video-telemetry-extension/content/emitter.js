...
const Emitter = (() => {
   ...
   */
  function emitUpdate(videoId, stateWithDelta) {
    const message = {
      type: "VIDEO_UPDATE",
      payload: {
        id: videoId,
    ...
    _sendWithRetry(message, MAX_RETRIES);
  }
   ...
   */
  function _sendWithRetry(message, retriesLeft) {
      ...
      chrome.runtime.sendMessage(message, (response) => {
        ...
        if (response?.status !== "ok") {
          ...
          if (retriesLeft > 0) {
            ...
            setTimeout(() => _sendWithRetry(message, retriesLeft - 1), 100);
          } else {
            console.error(
  ...
  return { emitUpdate, emitAdded, emitRemoved };
})();